        ...
        try {
            ...
            switch (notification.type) {
                case NOTIFICATION_TYPES.TRAFFIC_WARNING_5:
                    ...
                    message += `💡 Рекомендуем купить новый ключ, чтобы не остаться без VPN!`;
                    break;
                ...
                case NOTIFICATION_TYPES.TRAFFIC_WARNING_1:
                    ...
                    message += `Дней до окончания: ${notification.data.daysRemaining}\n\n`;
                    message += `⚡ Срочно купите новый ключ, иначе доступ будет заблокирован!`;
                    break;
                ...
                case NOTIFICATION_TYPES.TIME_WARNING_3:
                    message = `⏰ <b>Ключ скоро истекает!</b>\n\n`;
                    ...
                    message += `💡 Самое время купить новый ключ по выгодной цене!`;
                    break;

                case NOTIFICATION_TYPES.TIME_WARNING_1:
                    message = `🔥 <b>ВНИМАНИЕ: Ключ истекает завтра!</b>\n\n`;
                    message += `Остался всего ${notification.data.daysRemaining} день!\n`;
                    message += `Использовано трафика: ${notification.data.usagePercentage}%\n\n`;
                    message += `⚡ Купите новый прямо сейчас, чтобы не потерять доступ к VPN!`;
                    break;

                case NOTIFICATION_TYPES.TIME_EXPIRED:
                    message = `⛔ <b>Ключ истёк!</b>\n\n`;
                    message += `Срок действия VPN закончился.\n`;
                    message += `Доступ заблокирован.\n\n`;
                    message += `🛒 Купите новый ключ для продолжения использования VPN.`;
                    break;

                default:
                    message = `📢 Уведомление о состоянии вашего VPN ключа.`;
            }
